...
class UiAutomator2Server {
  ...
  async startInstrumentationProcess () {
    const cmd = ['nohup', 'am', 'instrument', '-w'];
    ...
    this.instrumentationProcess.once('exit', (code, signal) => {
      this.log.debug(`[Instrumentation] The process has exited with code ${code}, changing server ${process.env.DEVICE_IP}, port 6790`);
      this.jwproxy.server = process.env.DEVICE_IP || "localhost"
      this.jwproxy.port = 6790
